...
CARD_SOURCES = (
    "현대카드", "BC카드", "BC카드(신용)", "BC카드(체크)",
    "삼성카드", "KB카드", "신한카드", "롯데카드", "비씨카드",
)

# ── 업로드 파서 · 카테고리 분류 ───────────────────────
# streamlit에 의존하지 않는 순수 로직은 upload_parsers.py로 분리했다.
# cron(scripts/email_parser.py)이 메일 첨부를 같은 파서로 처리하기 위함 —
# app.py는 최상위에서 대시보드를 그려서 import할 수 없다.
# 여기서 되가져와 기존 참조(app.guess_category 등)를 그대로 유지한다.
from upload_parsers import (  # noqa: F401
    BIDIRECTIONAL_RULES,
    CATEGORY_KEYWORDS,
    HYUNDAI_COL_ALIASES,
    _BytesUpload,
    ...
    _get_owner_name,
    _match_column,
    _normalize_korean_date,
    _read_hyundai_html_as_df,
    find_bc_echo_rows,
    guess_category,
    parse_any_file,
    parse_hyundai_file,
    parse_ibk_account_file,
    parse_kakaobank_file,
)
...
def pair_self_transfers_in_sheet(tolerance_days: int = 2) -> tuple[int, int]:
    ...
    ibk_imy = []  # (row#, date, type, amount) — IBK 본인명의 거래
    kk_candidates = []  # 매칭 대상: 카뱅 본인명의 + 어머니차입금
    for i, row in enumerate(rows[1:], start=2):
        if len(row) <= max(date_idx, source_idx, type_idx, amount_idx, merch_idx, cat_idx):
            continue
        if row[merch_idx].strip() != owner:
            continue
        try:
...
# 모드 토글 — 정보 과부하 방지. 간단=핵심 메트릭만 / 회계사=모든 분석
VIEW_MODE = st.sidebar.radio(
    "📊 표시 모드",
    options=("회계사", "간단"),
    index=0,
    horizontal=True,
    help="간단 모드: 손익·카테고리·거래내역만. 회계사 모드: 전체 분석 (예측·이상치·청구매칭·연간결산 등)",
    key="view_mode",
)
SIMPLE_MODE = VIEW_MODE == "간단"
st.sidebar.markdown("---")
...
with st.sidebar.expander("📊 데이터 입력 현황", expanded=True):
    ...
    # 이번 달 데이터 누락 자동 감지 — 오늘 기준 이번 달 거래가 0건이면 경고
    today = pd.Timestamp.now().normalize()
    this_month_str = today.strftime("%Y-%m")
    stale_sources = []  # (출처, 최신 날짜) — 이번 달 거래 없는 활성 출처
    if not df_all.empty:
        df_active = df_all[df_all["출처"].isin(("IBK기업은행", "카카오뱅크", "현대카드"))]
        for src, grp in df_active.groupby("출처"):
            latest = grp["날짜"].max()
            if pd.isna(latest):
                continue
            # 활성 출처(최근 90일 안에 거래 있던 것)에 한해 이번 달 누락 체크
            if (today - latest).days > 90:
                continue
            if latest.strftime("%Y-%m") != this_month_str:
                stale_sources.append((src, latest.strftime("%Y-%m-%d"),
                                       (today - latest).days))
    if stale_sources:
        msg_lines = [f"⚠️ **이번 달({this_month_str}) 데이터 누락**"]
        for src, last, days in stale_sources:
            msg_lines.append(f"  • {src}: 최신 {last} ({days}일 전)")
        msg_lines.append("→ 최신 파일 재업로드 필요")
        st.warning("\n".join(msg_lines))
...
# ── 📱 모바일용 핵심 메트릭 (사이드바 접혀도 보이게) ───
if not df_all.empty:
    _mob_cols = st.columns(3)
    with _mob_cols[0]:
        mom_rows = df_all[df_all["카테고리"] == "어머니차입금"]
        if not mom_rows.empty:
            _b = mom_rows[mom_rows["유형"] == "입금"]["금액"].sum()
            _r = mom_rows[mom_rows["유형"] == "출금"]["금액"].sum()
            st.metric("🏠 어머니 부채 순", f"{_b - _r:+,.0f}원")
    with _mob_cols[1]:
        if "잔액" in df_all.columns:
            kk_rows_m = df_all[(df_all["출처"] == "카카오뱅크") & df_all["잔액"].notna()]
            if not kk_rows_m.empty:
                _kk_latest = int(kk_rows_m.sort_values("날짜").iloc[-1]["잔액"])
                st.metric("💛 카뱅 잔액", f"{_kk_latest:+,.0f}원")
    with _mob_cols[2]:
        try:
            _loan = load_loan_records()
            if not _loan.empty:
                _last_loan = _loan.iloc[-1]
                st.metric(
                    f"🏠 보금자리론 {int(_last_loan['회차'])}회차",
                    f"{int(_last_loan['잔액']):,}원",
                )
        except Exception:
            pass
...
# ── 🔮 3개월 현금흐름 예측 (회계사 모드 전용) ─────────
if not SIMPLE_MODE:
    pass  # 아래 forecast 블록 실행
# 회계사 모드일 때만 forecast/이상치/카드매칭 등 실행 — 들여쓰기 변경 없이
# SIMPLE_MODE 가드를 각 섹션 안에서 처리한다 (구조 안정성).
if not SIMPLE_MODE and not df_all.empty:
...
with col_up:
    uploaded_list = st.file_uploader(
        "현대카드 Excel/CSV/HTML 파일 (여러 개 가능)",
...
with col_info:
    st.markdown("""
    **현대카드 내역 내보내기 방법:**
    1. 현대카드 앱 → 이용내역 → 다운로드 아이콘 → Excel 저장
    2. 청구월별로 여러 파일을 **한꺼번에 드래그** 가능
    3. 자동 저장 켜져 있으면 즉시 시트 추가 (중복은 자동 제외)
    """)

uploaded = uploaded_list[0] if uploaded_list and len(uploaded_list) == 1 else None

if uploaded_list and len(uploaded_list) > 1:
    # 다중 파일 — 순차 파싱 + autosave + 결과 요약
    total_added = 0
    total_dup = 0
    results = []
    for f in uploaded_list:
        try:
            parsed = parse_hyundai_file(f)
        except Exception as e:
            results.append({"파일": f.name, "결과": f"❌ 파싱 실패: {e}", "건수": 0})
            continue
        if parsed is None or parsed.empty:
            results.append({"파일": f.name, "결과": "⚠️ 거래 없음", "건수": 0})
            continue
        _maybe_autosave(parsed, "현대카드", f)
        results.append({
            "파일": f.name,
            "결과": f"✅ {len(parsed)}건 / {parsed['금액'].sum():,.0f}원",
            "건수": len(parsed),
        })
        total_added += len(parsed)
    st.success(
        f"✅ {len(uploaded_list)}개 파일 처리 — 총 {total_added}건 시트에 추가 시도 "
        f"(자동 저장 토글 ON 시 중복 제외 후 실제 저장)"
    )
