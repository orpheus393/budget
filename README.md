...
- KB는 BC카드처럼 PDF가 아니라 **본문 HTML 안의 `var list_pe01Json = [...]` JavaScript 변수**에 개별 거래를 임베드
- `process_kb_statements()`가 매시간 cron에서 KB 명세서 메일을 검색해 본문 HTML에서 거래 추출 → 시트 저장 → 처리완료 폴더로 이동
- 비밀번호 불필요 (PDF가 아니므로)
- 청구월 정보(결제년월일)는 각 행의 `원문` 컬럼에 함께 기록되어 카드 청구 ↔ 사용 매칭에 활용
