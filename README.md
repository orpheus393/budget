...
## KB국민카드 (이메일 명세서 수신 권장)
KB국민카드는 모바일 앱에서 거래내역 다운로드를 지원하지 않으므로, 네이버 이메일로 명세서를 받도록 설정하면 자동 파싱 가능합니다.

### 사용자 설정
1. KB국민카드 홈페이지 / 앱 → **이용대금명세서 → 수령방법 → 이메일**
2. 수신 이메일을 `NAVER_EMAIL`로 등록한 네이버 주소로 변경

### 시스템 상태 (현재 placeholder)
- 발신자 패턴 `kbcard.com / kbcard.co.kr`은 등록됨 → 메일이 도착하면 인식
- ⚠️ **PDF 명세서 파서는 아직 미작성** — 첫 명세서 도착 후 형식 분석해 추가 예정
- PDF에 비밀번호가 있으면 `KB_PDF_PASSWORD` 시크릿 추가 (BC카드와 동일 패턴)
